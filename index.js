require('dotenv').config();
const TelegramBot = require('node-telegram-bot-api');
const { GoogleGenerativeAI } = require('@google/generative-ai');
const { initializeApp } = require('firebase/app');
const { 
    getFirestore, 
    collection, 
    addDoc, 
    doc, 
    setDoc, 
    getDoc, 
    updateDoc, 
    onSnapshot 
} = require('firebase/firestore');
const express = require('express');

// --- 1. ВЕБ-СЕРВЕР ДЛЯ RENDER (Health Check) ---
const app = express();
app.get('/', (req, res) => res.send('Aegis Bot Status: Online'));
const PORT = process.env.PORT || 10000;
app.listen(PORT, () => console.log(`Мониторинг запущен на порту ${PORT}`));

// --- 2. НАСТРОЙКИ ---
const CRM_APP_ID = process.env.CRM_CUSTOM_APP_ID || 'aegis-leads-app';

// Инициализация Telegram
const bot = new TelegramBot(process.env.TELEGRAM_BOT_TOKEN, { polling: true });

// Инициализация Gemini
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
// Используем базовую модель, которая поддерживается во всех регионах
const aiModel = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

// Инициализация Firebase
const firebaseConfig = {
    apiKey: process.env.FIREBASE_API_KEY,
    authDomain: process.env.FIREBASE_AUTH_DOMAIN,
    projectId: process.env.FIREBASE_PROJECT_ID,
    storageBucket: process.env.FIREBASE_STORAGE_BUCKET,
    messagingSenderId: process.env.FIREBASE_MESSAGING_SENDER_ID,
    appId: process.env.FIREBASE_APP_ID
};

const fbApp = initializeApp(firebaseConfig);
const db = getFirestore(fbApp);

const SYSTEM_PROMPT = `Ты — ИИ-ассистент юридической компании "ИДЖИС". 
Твоя цель: вежливо консультировать клиентов по списанию долгов. 
Отвечай кратко (2-3 предложения). Спрашивай сумму долга.`;

// --- 3. ЛОГИКА ТЕЛЕГРАМ ---
bot.on('message', async (msg) => {
    const chatId = String(msg.chat.id);
    const text = msg.text;

    if (!text || text.startsWith('/')) return;

    console.log(`[TG] Сообщение от ${chatId}: ${text}`);

    try {
        const leadRef = doc(db, 'artifacts', CRM_APP_ID, 'public', 'data', 'leads', chatId);
        const leadSnap = await getDoc(leadRef);
        const leadData = leadSnap.exists() ? leadSnap.data() : null;

        // 1. Сохраняем сообщение пользователя
        await addDoc(collection(db, 'artifacts', CRM_APP_ID, 'public', 'data', 'messages'), {
            chatId: chatId,
            sender: 'user',
            text: text,
            timestamp: Date.now()
        });

        // 2. Создаем/обновляем карточку лида
        await setDoc(leadRef, {
            name: msg.from.first_name + (msg.from.last_name ? ' ' + msg.from.last_name : ''),
            username: msg.from.username || 'n/a',
            updatedAt: Date.now(),
            status: leadData?.status || 'ai_active'
        }, { merge: true });

        // Если в чате оператор — бот не мешает
        if (leadData?.status === 'operator_active') return;

        // 3. Ответ ИИ
        bot.sendChatAction(chatId, 'typing');
        
        const result = await aiModel.generateContent(`${SYSTEM_PROMPT}\n\nКлиент: ${text}\nАссистент:`);
        const aiResponse = result.response.text();

        // Шлем в ТГ
        await bot.sendMessage(chatId, aiResponse);

        // Сохраняем ответ ИИ в базу
        await addDoc(collection(db, 'artifacts', CRM_APP_ID, 'public', 'data', 'messages'), {
            chatId: chatId,
            sender: 'ai',
            text: aiResponse,
            timestamp: Date.now()
        });

        // 4. Обновляем краткое резюме (саммари)
        const sumResult = await aiModel.generateContent(`Кратко опиши ситуацию клиента (одна фраза): ${text}`);
        await updateDoc(leadRef, {
            summary: sumResult.response.text()
        });

    } catch (err) {
        console.error("[ERROR] Ошибка в работе бота:", err.message);
    }
});

// --- 4. ПЕРЕСЫЛКА ИЗ CRM В ТЕЛЕГРАМ ---
onSnapshot(collection(db, 'artifacts', CRM_APP_ID, 'public', 'data', 'messages'), (snap) => {
    snap.docChanges().forEach(change => {
        if (change.type === 'added') {
            const m = change.doc.data();
            if (m.sender === 'operator') {
                bot.sendMessage(m.chatId, m.text).catch(e => console.error("Ошибка пересылки:", e.message));
            }
        }
    });
});

console.log(`Бот успешно запущен. Используемый ID: ${CRM_APP_ID}`);
